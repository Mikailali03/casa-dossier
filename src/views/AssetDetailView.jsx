...
import { 
  ...
  Shield, Trash2, Hammer, UserCheck, QrCode, Printer
} from 'lucide-react';
...
// --- MAIN VIEW ---
export default function AssetDetailView({ asset, activeProperty, setSelectedAsset, providers, onUpdate }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 font-sans">
      ...
      <div id="printable-label" className="hidden flex-col items-center text-center bg-white text-black p-4 border border-black font-sans">
        ...
        <QRCodeSVG value={qrProtocol} size={150} level="H" />
            ...
            <section>
              ...
              {asset.maintenance_tasks?.length > 0 ? (
                [...asset.maintenance_tasks].sort((a,b) => new Date(a.next_due_date) - new Date(b.next_due_date)).map(t => (
                  <MaintenanceTaskItem key={t.id} task={t} onComplete={(task) => { setSelectedTask(task); setShowLogModal(true); }} onDelete={deleteTask} />
                ))

...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900 border border-amber-500 p-4 shadow-2xl rounded-sm font-mono max-w-[240px]">
        <p className="text-amber-500 font-black text-[10px] uppercase mb-3 border-b border-amber-500/30 pb-2 tracking-widest">{data.year} Forecast</p>
        <div className="space-y-3">
          {data.assets.map((asset, idx) => (
            <div key={idx} className="flex flex-col border-l-2 border-slate-700 pl-3 text-left">
              <p className="text-white font-bold text-[9px] uppercase leading-tight">{asset.sub_category}</p>
              <p className="text-slate-500 text-[8px] uppercase tracking-tighter">Est. ${(asset.replacement_cost_est || 0).toLocaleString()}</p>
            </div>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-12 animate-in fade-in duration-500 font-sans">
      ...
      <div className="max-w-6xl mx-auto mb-6 md:mb-10 flex flex-col md:flex-row justify-between items-start md:items-end border-b border-slate-800 pb-8 gap-6">
        <div className="text-left">
          <button onClick={onClose} className="flex items-center gap-2 text-slate-500 hover:text-amber-500 text-[10px] font-black uppercase tracking-widest mb-4 transition-colors font-mono"><ArrowLeft className="w-4 h-4" /> Dashboard</button>
          <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter flex items-center gap-4"><TrendingUp className="w-6 h-6 md:w-8 md:h-8 text-amber-500" /> Capital Outlook</h1>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-4 md:p-6 rounded-sm shadow-xl flex items-center gap-4 md:gap-6 w-full md:w-auto">
          <div className="text-left border-r border-slate-800 pr-4 md:pr-6 flex-grow">
            <p className="text-[8px] md:text-[10px] font-black text-slate-600 uppercase tracking-widest mb-1 font-mono">10Y Liability Horizon</p>
      ...
      <div className="max-w-6xl mx-auto bg-slate-900/40 border border-slate-800 p-4 md:p-8 rounded-sm shadow-2xl relative h-[350px] md:h-[450px]">
        <div className="absolute top-2 left-4 flex items-center gap-2 opacity-50">
           ...
           <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest">Active Analysis</span>
        </div>
        
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 20, right: 10, left: -20, bottom: 20 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
            <XAxis dataKey="year" stroke="#475569" fontSize={9} fontWeight="900" tickLine={false} axisLine={false} dy={10} />
            <YAxis stroke="#475569" fontSize={9} fontWeight="900" tickLine={false} axisLine={false} tickFormatter={(v) => `$${v}`} dx={0} />
            <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#f59e0b', strokeWidth: 1, strokeDasharray: '4 4' }} />
            <Line type="stepAfter" dataKey="spend" stroke="#f59e0b" strokeWidth={3} dot={{ fill: '#f59e0b', r: 4, strokeWidth: 0 }} activeDot={{ r: 6, fill: '#fff', stroke: '#f59e0b', strokeWidth: 2 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="max-w-6xl mx-auto mt-6 text-left">
        <div className="p-4 bg-slate-900 border border-slate-800 text-[9px] font-mono text-slate-500 uppercase leading-relaxed tracking-tighter">
           Projection based on industry useful-life standards and AI-estimated replacement costs. Accurate budgeting requires periodic professional onsite inspections.
        </div>
      </div>
    </div>
  );
